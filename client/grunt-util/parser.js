// cloned from https://github.com/ludohenin/gulp-inline-ng2-template.git
...
module.exports = function parser(file, options) {
  ...
  function getFragment() {
    ...
    // rather want to check fot moduleId...but too complicated
    // -> templateUrl needs to start with ./
    if (frag && frag.indexOf('"./') < 0 && frag.indexOf('\'./') < 0 && frag.indexOf('`./') < 0) {
      frag = undefined;
    }
  ...
  function replaceFrag() {
    ...
    function stringToString() {
      // return '`' + startOfInsertionBlock + assetFiles + endOfInsertionBlock + indentation + '`';
      var s = startOfInsertionBlock + assetFiles + endOfInsertionBlock + indentation;
      return JSON.stringify(s);
    }

    // Build the final string.
    if ('html' === opts.type)
      assetFiles = opts.prop + ': ' + stringToString();
    if ('css' === opts.type)
      assetFiles = opts.prop + ': [' + stringToString() + ']';
